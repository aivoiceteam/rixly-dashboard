...
async function callWithRetry<T>(
  ...
  const getRetryDelayMs = (errorText: string): number => {
    ...
    const plainSecondsMatch = errorText.match(/retry in\s+([0-9]+(?:\.[0-9]+)?)s?/i);
  ...
  if (
    ...
    finalErrorLower.includes("api_key_invalid") ||
    finalErrorLower.includes("invalid api key")
  ) {
    throw new Error(
      `Gemini API key is invalid. Add a valid Google AI Studio key in Settings (Additional Gemini API Keys) or set GEMINI_API_KEY in environment. Details: ${finalError}`
    );
  }
